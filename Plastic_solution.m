function [] = Plastic_solution(nelem,tau)
...
for d = 1:1/tau
    ...
    for k = 1:100
        ...
        if (norm(delta_u,inf)<=0.005*norm(u,inf) | norm(Gk_red,inf)<=0.005*norm(Gk,inf))
            X = ['Element = ',num2str(nelem),' Step no ',num2str(d),' at Volumatric Strain = ',num2str(d*tau*epsilon_v),' and Uo = ',num2str(u(1,1)),' Number of iteration required to converge is ',num2str(k-1),'.'];
            disp(X);
            break;
        end
    end
...
%for j = 1:nelem+1
%    [u_exact] = exact_solution_u(rnodes(j),epsilon_v, ri, E);
%    u_array(j,1) = u_exact;
